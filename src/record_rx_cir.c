...
#define APP_NAME "4 message ranging with CIR v1.0"
...
#define ID_IDX      2
#define MSGNO_IDX   3   // all messages
...
// temporary function definition. get rid of this once parsing is properly implemented
int test(int argc, char* argv[]) {
    ...
    if(argc == 4) {
        if(!strcmp(argv[1], "INIT")) {
            mode = 'I';
            cir = 0;
        } else if (!strcmp(argv[1], "INITC")) {
            mode = 'I';
            cir = 1;
        } else if (!strcmp(argv[1], "RESP")) {
            mode = 'R';
            cir = 0;
        } else if (!strcmp(argv[1], "RESPC")) {
            mode = 'D';
            cir = 1;
        } else {
            mode = 'U';
        ...
        printf("usage: %s INIT/RESP id count\n", argv[0]);
        exit(1);
    }
    ...
    while (num_initiations < count || count < 0)
    {
        ...
        if (status_reg & SYS_STATUS_RXFCG) {
            ...
            if (memcmp(rx_buffer, rx2_msg, 4) == 0) {
                ...
                delayed_tx_time = next_delayed_tx(timestamps.rx_timestamp[1], RX_TO_TX_DLY_UUS);
...
static void responderTask(uint8_t id, int count, uint8_t cir) {
    ...
    // allocate memory
    rxInfo = (struct completeChannelInfo *) malloc(2*sizeof(struct completeChannelInfo));
    if (rxInfo == NULL) {
        printf("Error: could not allocate memory\n");
        exit(1);
    }
...
static double get_ds_tof(struct ts_struct *timestamps) {
    ...
    poll_rx_ts_32 = (uint32)timestamps->rx_timestamp[0];
    resp_tx_ts_32 = (uint32)timestamps->tx_timestamp[1];
    final_rx_ts_32 = (uint32)timestamps->rx_timestamp[2];
    Ra = (double)(timestamps->rx_timestamp[1] - timestamps->tx_timestamp[0]);
    Rb = (double)(final_rx_ts_32 - resp_tx_ts_32);
    Da = (double)(timestamps->tx_timestamp[2] - timestamps->rx_timestamp[1]);
...
static void saveChannelInfoToFile(char *filename, struct completeChannelInfo *info, struct ts_struct *ts, int type, int nInfo)
{
    ...
    if (output_file == NULL){
        ...
        if (type == 1) {
            ...
            tof = get_tof(ts, get_clock_offset(info[0].carrierintegrator));
        ...
        for (i = 0; i < nInfo; i++) {
            ...
            fprintf(output_file, "carrierInt,%ld\n", info[i].carrierintegrator);
            fprintf(output_file, "clockOffset,%f\n", get_clock_offset(info[i].carrierintegrator));

            fprintf(output_file, "CIRIQ\n");
            for (j = 0; j < CIR_SAMPLES; j++)
            {
                fprintf(output_file, "%d,%d\n", info[i].cir[j].real, info[i].cir[j].img);
            }
        }
